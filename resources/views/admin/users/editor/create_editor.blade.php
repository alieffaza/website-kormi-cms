    ...
    <div class="row justify-content-center">
        <div class="col-12 px-0">
            <div class="card px-0" style="border-radius:1rem; margin:0;">
                <div class="card-header"><h5 class="mb-0">Tambah Editor</h5></div>
                <div class="card-body mx-4 mt-0">
                    <form method="POST" action="{{ route('users.store_editor') }}">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3 mt-0">
                                    <label for="name" class="form-label">Nama Lengkap</label>
                                    <input type="text" class="form-control border border-secondary px-3 py-2" id="name" name="name" value="{{ old('name') }}" required>
                                    @error('name')<span class="text-danger text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div class="mb-3">
                                    <label for="email" class="form-label">Email</label>
                                    <input type="email" class="form-control border border-secondary px-3 py-2" id="email" name="email" value="{{ old('email') }}" required>
                                    @error('email')<span class="text-danger text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div class="mb-3">
                                    <label for="phone" class="form-label">No. Telepon</label>
                                    <input type="text" class="form-control border border-secondary px-3 py-2" id="phone" name="phone" value="{{ old('phone') }}" required>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3 position-relative">
                                    ...
                                    <input type="password" class="form-control border border-secondary px-3 py-2 pr-5" id="password" name="password" required>
                                ...
                                <div class="mb-3 position-relative">
                                    ...
                                    <input type="password" class="form-control border border-secondary px-3 py-2 pr-5" id="password_confirmation" name="password_confirmation" required>
                        ...
                        <div class="text-end">
                            ...
                            <a href="{{ route('users.editors') }}" class="btn btn-secondary">Batal</a>
                        </div>
                    </form>
                </div>
            </div>
